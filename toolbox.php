...
function TP_AJAX_wrapper_getProducts() {
	...
	try {
		...
		$prods = get_posts( 'post_type=' . tp_get_post_type() . '&post_status=publish,draft,pending&meta_key=tp_product_ID&numberposts='.intval($perBatch).'&offset='.intval($page*$perBatch) );
		$ids = array();
...
function TP_AJAX_wrapper_updateProduct() {
	...
	try {
		...
		$new_id = tp_add_product_from_feed( $product->id, $product->product_store_id );
